...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun contains(p: Point): Boolean {
        val eps = 1e-6
        return center.distance(p) < radius || abs(center.distance(p) - radius) <= eps
    }
}
...
fun diameter(vararg points: Point): Segment {
    ...
    var distance = 0.0
    var segment = Segment(begin = Point(0.0, 0.0), end = Point(0.0, 0.0))
    for (i in points.indices)
        for (j in 1 until points.size)
            if (points[i].distance(points[j]) > distance) distance = points[i].distance(points[j])
    for (i in 0 until points.size - 1)
        for (j in 1 until points.size)
            if (points[i].distance(points[j]) == distance) segment = Segment(begin = points[i], end = points[j])
    return segment
}
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        ...
        val divisor = otherSin * thisCos - thisSin * otherCos
        return Point(
            (b * otherCos - other.b * thisCos) / divisor,
            (b * otherSin - other.b * thisSin) / divisor
        )
    }
 ...
 */
fun lineByPoints(a: Point, b: Point): Line = Line(b, (atan2(b.y - a.y, b.x - a.x) + PI) % PI)
