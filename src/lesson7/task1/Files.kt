...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val variables = substrings.toSet()
    for (string in substrings)
        map[string] = 0
    for (line in File(inputName).readLines().map { it.toLowerCase() })
        for (string in variables) {
            var index = line.indexOf(string.toLowerCase())
            while (index != -1) {
                map[string] = map[string]!! + 1
                index = line.indexOf(string.toLowerCase(), index + 1)
            }
        }
    return map
}
...
fun centerFile(inputName: String, outputName: String) {
    ...
    if (lines.isNotEmpty()) {
        ...
        val maxSymbols = lines.maxByOrNull { it.length }?.length
        if (maxSymbols != null) for (line in lines) {
            file.write(" ".repeat((maxSymbols - line.length) / 2) + line)
            file.newLine()
        }
    }
    file.close()
