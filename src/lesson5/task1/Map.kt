...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((key, value) in mapA) result[key] = value
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val resultMap = mutableMapOf<String, MutableSet<Double>>()
    val result = mutableMapOf<String, Double>()
    for ((string, double) in stockPrices) resultMap.getOrPut(string) { mutableSetOf() }.add(double)
    for ((string, double) in resultMap) result[string] = double.sum() / double.size
    return result
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((map, pair) in stuff) {
        if (pair.second < min1 && pair.first == kind) {
            min1 = pair.second
            result = map
        }
    }
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for ((index, element) in list.withIndex())
        if (number - element != number / 2) list1.add(number - element) else list2.add(index)
    if (list2.size > 1) result = list2[0] to list2[1]
        ...
        for ((index, element) in list.withIndex())
            if (element == breaker) result = index1 to index
    }
    return result
